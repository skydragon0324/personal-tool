...
def get_note_or_404(db: Session, note_id: uuid.UUID) -> Note:
...
def get_note(db: Session, note_id: uuid.UUID) -> NoteRead:
    return NoteRead.model_validate(get_note_or_404(db, note_id))


def update_note(db: Session, note_id: uuid.UUID, payload: NoteUpdate) -> NoteRead:
